...
export default function RequestForm({ open, setOpen, t, profile, lang }) {
  const [form, setForm] = useState({ name: '', phone: '', type: t('forms.requestTypes')[0], subject: '', description: '' })
  if (!open) return null
  const submit = (e) => {
    e.preventDefault()
  ...
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setOpen(false)}><div className="modal"><header><div><span className="section-kicker"><ClipboardPenLine size={14} /> REQUEST</span><h3>{t('forms.requestTitle')}</h3></div><button onClick={() => setOpen(false)} aria-label={t('forms.close')}><X /></button></header><form onSubmit={submit}><label>{t('forms.name')}<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></label><label>{t('forms.phone')}<input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} required /></label><label>{t('forms.type')}<select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>{t('forms.requestTypes').map((x) => <option key={x}>{x}</option>)}</select></label><label>{t('forms.subject')}<input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} required /></label><label>{t('forms.description')}<textarea rows="5" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required /></label><button className="btn primary full" type="submit">{t('forms.sendWhatsApp')}</button></form></div></div>
}
